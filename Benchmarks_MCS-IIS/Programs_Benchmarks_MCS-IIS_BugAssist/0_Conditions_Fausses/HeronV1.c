 ...
 */
	...
	int HeronV1 (int i, int j, int k) {__CPROVER_assume(i==3 && j==4 && k==3);
		int trityp = 0;int result;
		int res = 0;
				...
				res = (3*i*i*i*i)/16;
				    ...
				    if (trityp == 1 && (i+j) > k) { // isosceles
						...
						if (trityp == 2 && (i+k) > j) {
							trityp = 2;
							// i==k
						        res = ((i+j+k)/2)*(((i+j+k)/2)-i)*(((i+j+k)/2)-j)*(((i+j+k)/2)-j); /*error, the instruction should be: res = ((i+j+k)/2)*(((i+j+k)/2)-i)*(((i+j+k)/2)-j)*(((i+j+k)/2)-i); */  
						}
						else {
							if (trityp == 3 && (j+k) > i) {
								trityp = 2;
