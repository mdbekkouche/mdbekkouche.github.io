...
void TcasKO5(int Cur_Vertical_Sep,int High_Confidence,int Two_of_Three_Reports_Valid,int Own_Tracked_Alt,int Own_Tracked_Alt_Rate,int Other_Tracked_Alt,int Alt_Layer_Value,int Up_Separation,int Down_Separation,int Other_RAC,int Other_Capability,int Climb_Inhibit)
	...
	DOWNWARD_RA = 2;
        ...
        enabled = (High_Confidence) && (Own_Tracked_Alt_Rate <= OLEV);/* && (Cur_Vertical_Sep > MAXALTDIFF); missing code, the instruction should be : enabled = (High_Confidence) && (Own_Tracked_Alt_Rate <= OLEV) && (Cur_Vertical_Sep > MAXALTDIFF); */ 
