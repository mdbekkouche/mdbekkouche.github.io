...
void TcasKO34(int Cur_Vertical_Sep,int High_Confidence,int Two_of_Three_Reports_Valid,int Own_Tracked_Alt,int Own_Tracked_Alt_Rate,int Other_Tracked_Alt,int Alt_Layer_Value,int Up_Separation,int Down_Separation,int Other_RAC,int Other_Capability,int Climb_Inhibit)
	...
	alt_sep = UNRESOLVED;
        if (enabled && tcas_equipped && intent_not_known || !tcas_equipped) /* error in the condition, the instruction should be : if (enabled && ((tcas_equipped && intent_not_known) || !tcas_equipped))*/
	    { 
		if (Climb_Inhibit){
