...
void TcasKO8(int Cur_Vertical_Sep,int High_Confidence,int Two_of_Three_Reports_Valid,int Own_Tracked_Alt,int Own_Tracked_Alt_Rate,int Other_Tracked_Alt,int Alt_Layer_Value,int Up_Separation,int Down_Separation,int Other_RAC,int Other_Capability,int Climb_Inhibit)
	{ __CPROVER_assume((Cur_Vertical_Sep==735) &&(High_Confidence==1)&&(Two_of_Three_Reports_Valid==0) && (Own_Tracked_Alt == 2792)&&(Own_Tracked_Alt_Rate == 119)&&(Other_Tracked_Alt == 224)&&(Alt_Layer_Value == 3)&&(Up_Separation == 739)&&(Down_Separation == 739)&&(Other_RAC == 0)&&(Other_Capability == 0)&&(Climb_Inhibit == 0));int Positive_RA_Alt_Thresh[4];
        int OLEV;
	int MAXALTDIFF;
	int MINSEP;
	int NOZCROSS;
	...
	DOWNWARD_RA = 2;
        ...
        Positive_RA_Alt_Thresh[3] = 700; /* constant mutation, the instruction should be :  Positive_RA_Alt_Thresh[3] = 740; */
