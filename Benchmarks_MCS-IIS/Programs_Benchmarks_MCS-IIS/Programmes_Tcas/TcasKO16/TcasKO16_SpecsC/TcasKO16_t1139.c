...
void TcasKO16(int Cur_Vertical_Sep,int High_Confidence,int Two_of_Three_Reports_Valid,int Own_Tracked_Alt,int Own_Tracked_Alt_Rate,int Other_Tracked_Alt,int Alt_Layer_Value,int Up_Separation,int Down_Separation,int Other_RAC,int Other_Capability,int Climb_Inhibit)
	{ __CPROVER_assume((Cur_Vertical_Sep==1058) &&(High_Confidence==1)&&(Two_of_Three_Reports_Valid==0) && (Own_Tracked_Alt == 1597)&&(Own_Tracked_Alt_Rate == 174)&&(Other_Tracked_Alt == 3253)&&(Alt_Layer_Value == 0)&&(Up_Separation == 399)&&(Down_Separation == 400)&&(Other_RAC == 1)&&(Other_Capability == 0)&&(Climb_Inhibit == 1));int Positive_RA_Alt_Thresh[4];
        int OLEV;
	int MAXALTDIFF;
	int MINSEP;
	int NOZCROSS;
