...
void TcasKO37(int Cur_Vertical_Sep,int High_Confidence,int Two_of_Three_Reports_Valid,int Own_Tracked_Alt,int Own_Tracked_Alt_Rate,int Other_Tracked_Alt,int Alt_Layer_Value,int Up_Separation,int Down_Separation,int Other_RAC,int Other_Capability,int Climb_Inhibit)
	{ __CPROVER_assume((Cur_Vertical_Sep==775) &&(High_Confidence==1)&&(Two_of_Three_Reports_Valid==1) && (Own_Tracked_Alt == 2961)&&(Own_Tracked_Alt_Rate == 373)&&(Other_Tracked_Alt == 5279)&&(Alt_Layer_Value == 3)&&(Up_Separation == 640)&&(Down_Separation == 499)&&(Other_RAC == 0)&&(Other_Capability == 1)&&(Climb_Inhibit == 0));int Positive_RA_Alt_Thresh[4];
        int OLEV;
	int MAXALTDIFF;
	int MINSEP;
	int NOZCROSS;
